const Hoek = require("@hapi/hoek");

const COOKIE_DEFAULTS = {
	isSameSite: "Lax",
	path: "/",
	ttl: 3600 * 1000,
	encoding: "iron",
	isSecure: true,
	clearInvalid: true,
};

module.exports =
	({ cookieName, callbackUrl, xhrCallbackUrl, scope, client }) =>
	(server, schemeOptions) => {
		const schemeConfig = Hoek.applyToDefaults(COOKIE_DEFAULTS, schemeOptions);
		server.state(cookieName, schemeConfig);
		return {
			authenticate: async (request, h) => {
				const oidc = request.state[cookieName];
				if (oidc?.credentials) {
					const { credentials, artifacts } = oidc;
					return h.authenticated({ credentials, artifacts });
				} if (request.auth.mode !== 'required') {
					return h.unauthenticated(null)
				}

				const state = request.route.path;
				h.state(cookieName, { state });

				if(request.headers['x-requested-with'] !== 'XMLHttpRequest') {
					const redirectUrl = client.authorizationUrl({
						redirect_uri: callbackUrl,
						scope,
						state,
					});

					return h.redirect(redirectUrl).takeover();
				} else {
					const redirectUrl = client.authorizationUrl({
						redirect_uri: xhrCallbackUrl,
						scope,
						state,
					});

					return h.response({code: 302, redirectUrl, callbackUrl}).takeover();
				}
			},
		};
	};
